...
function custom_admin_default_page() {
  ...
  // http://www.example.com/custom-user-dashboard.
    $GLOBALS['url'] = wp_get_referer();
	$current_page = $_SERVER['REQUEST_URI']; 
	if ( $GLOBALS['url'] == "http://luganskaid.org/dev/wp-login.php?redirect_to=http%3A%2F%2Fluganskaid.org%2Fdev%2Fdonation-form%2F" ) {
		return home_url('/dev/donation-form/');
	} elseif ( $GLOBALS['url'] == "http://luganskaid.org/dev/wp-login.php" ) {
		return home_url('');
	}
}
