...
*/
	...
	public static function preview() {
		...
		if ( self::$donation_id ) {
			...
			setup_postdata( $post );
			?>
			<script>$( document ).ready(function() {document.getElementById("donation_preview_submit_button").focus();});</script>
			<form method="post" id="donation_preview" target="_top">
				<input type="button" name="edit_donation" class="btn btn-warning btn-lg" onclick="window.history.back()" value="<?php _e( '&larr; Edit donation', 'wpdonations' ); ?>" />
				<input type="submit" class="btn btn-success btn-lg" name="continue" id="donation_preview_submit_button" class="button" value="<?php echo apply_filters( 'submit_donation_step_preview_submit_text', __( 'Submit Donation &rarr;', 'wpdonations' ) ); ?>" />
				...
				<div class="donation_preview single_donation">
					<h3><?php _e( 'Confirm your', 'wpdonations' ); ?> <?php the_title(); ?></h3>
