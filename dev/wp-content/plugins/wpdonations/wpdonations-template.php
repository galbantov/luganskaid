 ...
 * @return void
 */
function the_donors( $before = '', $after = '', $order = 'DESC' ) {
	...
	if( $post_count > 0) :
		...
		echo '<ul class="donors_list_by_amount">';
		// Loop
		while ($query->have_posts()) : $query->the_post();
		
			$donor_id = get_the_author_meta( 'user_id', get_the_ID() );
			echo "<li>" . get_the_author_meta( 'user_nicename', $donor_id ) . "</li>";
		...
		_e( 'No donors found', 'wpdonations');
